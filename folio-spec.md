...
- **European ticker prices:** Tickers with `.DE`, `.MI`, `.F`, `.L`, `.SW` suffixes resolve correctly from ISINs but often return no quote from FMP `/quote` on the current plan. This is an FMP tier limitation — upgrading to Premium adds European exchange quotes. Not a code bug.
