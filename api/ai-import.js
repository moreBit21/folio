...
export default async function handler(req, res) {
  ...
  const { content, fileType, fileName, brokerHint, brokerLabel } = req.body || {};
  ...
  // If broker is known and not "other", do a web search first to get format info
  let brokerFormatContext = "";
  if (brokerHint && brokerHint !== "other" && brokerLabel) {
    try {
      const searchRes = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: "You are a technical researcher. Search for information about broker/exchange CSV export formats and return a concise technical summary of the column structure. Return only the format description, no preamble.",
          messages: [{
            role: "user",
            content: `Search for the CSV export format used by "${brokerLabel}" for transaction history or portfolio exports. Find the column names, what data each column contains, and what transaction types are used. Summarize the format in 200 words or less.`
          }],
        }),
      });
      if (searchRes.ok) {
        const searchData = await searchRes.json();
        const text = (searchData.content || []).filter(b => b.type === "text").map(b => b.text).join("");
        if (text.trim()) brokerFormatContext = `\n\nWEB-SEARCHED FORMAT INFO FOR ${brokerLabel.toUpperCase()}:\n${text.trim()}`;
      }
    } catch(e) {
  ...
  const brokerHintLine = brokerHint && brokerHint !== "other" && brokerLabel
    ? `\nUSER-SELECTED BROKER: "${brokerLabel}" — prioritize this broker's known format when parsing.`
    : "\nBROKER: Unknown — auto-detect from file structure.";
...
You MUST respond with ONLY a valid JSON object — no explanation, no markdown, no code fences.${brokerHintLine}${brokerFormatContext}
...
${sampledContent}`;
  ...
  try {
    ...
    if (!parsed.broker) parsed.broker = brokerLabel || "Unknown";
